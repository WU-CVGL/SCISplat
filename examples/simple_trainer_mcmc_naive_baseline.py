...
from datasets.colmap_dataparser import ColmapParser, ValColmapParser
...
class Runner:
    ...
    def __init__(self, cfg: Config) -> None:
        ...
        self.parser = ColmapParser(
            ...
            downsample_rate=None,  # not used,
            downsample_cap=10000,  # the best setting for most,
        )

        self.val_parser = ValColmapParser(
            data_dir=cfg.data_dir,
            factor=cfg.data_factor,
            scale_factor=cfg.scale_factor,
        ...
        self.valset = Dataset(self.val_parser, split="all")
    ...
    def eval(self, step: int):
        ...
        if cfg.novel_view:
            ...
            Tstart = pp.mat2SE3(self.valset[0]["camtoworld"]).Log()
            T2 = pp.mat2SE3(self.valset[1]["camtoworld"]).Log()
            T3 = pp.mat2SE3(self.valset[2]["camtoworld"]).Log()
            T4 = pp.mat2SE3(self.valset[3]["camtoworld"]).Log()
            T5 = pp.mat2SE3(self.valset[4]["camtoworld"]).Log()
            T6 = pp.mat2SE3(self.valset[5]["camtoworld"]).Log()
            T7 = pp.mat2SE3(self.valset[6]["camtoworld"]).Log()
            Tend = pp.mat2SE3(self.valset[7]["camtoworld"]).Log()
        ...
        if psnr.item() > self.best_psnr:
            ...
            for i, rgb in enumerate(rgb_buffer):
                imageio.imwrite(
                    f"{self.best_dir}/rgb_{i:04d}.png", (rgb * 255).astype(np.uint8)
                )
            if cfg.novel_view:
                color_save_novel_view, _, _ = self.rasterize_splats(
                ...
                color_save_novel_view = torch.clamp(color_save_novel_view, 0.0, 1.0)
