...
from bayer_utils import rgb_to_bayer_rggb, rgb_to_bayer_grbg
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            for data in trainloader:
                ...
                if "garage" in os.path.basename(self.cfg.data_dir):
                    colors = rgb_to_bayer_grbg(colors).to(device).unsqueeze(-1)
                elif "truck" in os.path.basename(self.cfg.data_dir):
                    colors = rgb_to_bayer_rggb(colors).to(device).unsqueeze(-1)
            ...
            if hasattr(self.parser, "comp_img"):
                comp_gt_img = self.parser.comp_img.to(device).unsqueeze(-1)
            else:
